...
import 'counter_sale_payment_summary.dart';

class CounterSaleCartScreen extends StatefulWidget {
  const CounterSaleCartScreen({super.key});

  @override
  _CounterSaleCartScreenState createState() => _CounterSaleCartScreenState();
}

class _CounterSaleCartScreenState extends State<CounterSaleCartScreen> {
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                border: Border.all(
                                  color: item['isVeg'] ? const Color(0xFF00A35E) : const Color(0xFFE23D3D),
                                  width: 2,
                                ),
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    if (item['oldPrice'] != null) const SizedBox(width: 6),
                                    Text(
                                      '₹${item['price']}',
                                      style: const TextStyle(
                                        color: Colors.black,
                ...
                children: [
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          // Build items with qty for summary
                          final itemsForSummary = <Map<String, dynamic>>[];
                          for (int i = 0; i < cartItems.length; i++) {
                            itemsForSummary.add({
                              'name': cartItems[i]['name'],
                              'subtitle': cartItems[i]['subtitle'],
                              'price': cartItems[i]['price'],
                              'qty': quantities[i],
                            });
                          }
                          final total = getTotalAmount();
                          final gst = (total * 0.10).round();
                          final vat = 0;
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => CounterSalePaymentSummaryScreen(
